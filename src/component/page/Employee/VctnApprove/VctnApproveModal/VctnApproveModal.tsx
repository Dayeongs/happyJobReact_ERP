...
export interface IVctnApproveModalProps {
    vctnSeq?: number;
    onPostSuccess: () => void;
    setVctnSeq: (vctnSeq: undefined) => void;
}

export const VctnApproveModal: FC<IVctnApproveModalProps> = ({ vctnSeq, onPostSuccess, setVctnSeq }) => {
    const [modal, setModal] = useRecoilState(modalState);
    const [rejectReasonTr, setRejectReasonTr] = useState<boolean>(false);
    const [vctnDetail, setVctnDetail] = useState<IVctn>();

    // 주의! [modal, vctnSeq] modal 처럼 자주 변경되는 값을 의존성 배열에 넣으면 무한루프에 빠지기 쉽다.
    useEffect(() => {
        searchVctnDetail();
    }, [vctnSeq]);
